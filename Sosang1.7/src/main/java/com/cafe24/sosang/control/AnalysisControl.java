...
public class AnalysisControl {
	...
	public String showData(HttpSession session, AreaDTO adto) throws Exception {
	...
	public String overallAnalysis(AreaDTO adto, Model model, HttpSession session) {
		...
		double gtot = overallService.getSum(g1,g2,g3,g4);
		double stot = overallService.getSum(s1,s2,s3,s4);
		double btot = overallService.getSum(b1,b2,b3,b4,b5,b6);
		double ptot = overallService.getSum(p1,p2,p3);
		double htot = overallService.getSum(h1,h2,h3);
		double total = overallService.getSum(gtot,stot,btot,ptot,htot);
		
		//등급
		model.addAttribute("rank", overallService.getRank(total));
		
		//총점
		model.addAttribute("gtot", gtot);
		model.addAttribute("stot", stot);
		model.addAttribute("btot", btot);
		model.addAttribute("ptot", ptot);
		model.addAttribute("htot", htot);
		...
		//이전 분기
		double[] bs1 = overallService.getBeforeScores1(adto, 1);
		double[] bs2 = overallService.getBeforeScores2(adto, 2);
		double[] bs3 = overallService.getBeforeScores3(adto, 3);
		double[] bs4 = overallService.getBeforeScores4(adto, 4);
		...
		model.addAttribute("before_total1", overallService.getSum(bs1));
		model.addAttribute("before_total2", overallService.getSum(bs2));
		model.addAttribute("before_total3", overallService.getSum(bs3));
		model.addAttribute("before_total4", overallService.getSum(bs4));
