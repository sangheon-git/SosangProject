...
public class OverallServiceImpl implements OverallService {
	...
	public double checkScore(double score, double max) {
		if(score < 0) {
			score = 0;
		}
		if(score > max) {
	...
	public AreaDTO getBeforeQuarter(AreaDTO adto, int cnt) {
		AreaDTO dto = new AreaDTO();
		int year, quarter;
		if(adto.getSelect_quarter() < cnt+1) {
			year = adto.getSelect_year()-1;
			quarter = adto.getSelect_quarter()+4-cnt;
		}
		else {
			year = adto.getSelect_year();
			quarter = adto.getSelect_quarter()-cnt;
		}
	...
	@Override
	public double[] getBeforeScores1(AreaDTO adto, int cnt) {
		AreaDTO dto = getBeforeQuarter(adto, 1);
		return getTotals(dto);
	}

	@Override
	public double[] getBeforeScores2(AreaDTO adto, int cnt) {
		AreaDTO dto = getBeforeQuarter(adto, 2);
		return getTotals(dto);
	}

	@Override
	public double[] getBeforeScores3(AreaDTO adto, int cnt) {
		AreaDTO dto = getBeforeQuarter(adto, 3);
		return getTotals(dto);
	}

	@Override
	public double[] getBeforeScores4(AreaDTO adto, int cnt) {
		AreaDTO dto = getBeforeQuarter(adto, 4);
		return getTotals(dto);
	}
